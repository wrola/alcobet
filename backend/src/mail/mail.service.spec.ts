  ...
  describe('sendBetCompletedEmail', () => {
    it('should send success email when bet is completed successfully', async () => {
      ...
      expect(mockTransporter.sendMail).toHaveBeenCalledWith({
        ...
        html: expect.stringContaining('Test User successfully completed their alcohol abstinence commitment'),
      });
      expect(result).toBe(true);
    });
    ...
    it('should send failure email when bet fails', async () => {
      ...
      expect(mockTransporter.sendMail).toHaveBeenCalledWith({
        ...
        html: expect.stringContaining('Test User\'s alcohol abstinence commitment ended'),
      });
      expect(result).toBe(true);
    });
