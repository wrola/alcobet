...
describe("TrustmanController", () => {
  ...
  describe("getResponsePage", () => {
    it("should throw NotFoundException for invalid token", async () => {
      ...
      await expect(controller.getResponsePage("invalid-token")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("should throw BadRequestException for expired token", async () => {
      ...
      await expect(controller.getResponsePage("expired-token")).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("submitResponse", () => {
    it("should record clean response successfully", async () => {
      ...
      const result = await controller.submitResponse("valid-token", {
        response: "clean",
      });

      expect(result.message).toBe("Response recorded successfully");
      expect(result.response).toBe("clean");
      expect(mockBetsService.updateStatus).not.toHaveBeenCalled(); // Should not mark as failed for clean response
    });
    ...
    it("should record drank response and mark bet as failed", async () => {
      ...
      expect(mockBetsService.updateStatus).toHaveBeenCalledWith(1, "failed");
      expect(mockMailService.sendBetCompletedEmail).toHaveBeenCalledWith(
        validCheck.bet,
        false,
      );
    });
    ...
    it("should reject duplicate response submission", async () => {
      ...
      await expect(
        controller.submitResponse("valid-token", { response: "clean" }),
