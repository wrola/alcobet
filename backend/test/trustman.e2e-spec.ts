...
describe('Trustman API (e2e)', () => {
  ...
  describe('/trustman/response/:token (GET)', () => {
    it('should return daily check details for valid token', async () => {
      // Create test data
      const user = { id: 1, email: 'test@example.com', name: 'Test User', googleId: 'google123' };
      const bet = { id: 1, user, trustmanEmail: 'trustman@example.com', amount: 100, deadline: new Date('2024-12-31') };
      const dailyCheck = {
        id: 1,
        bet,
        checkDate: new Date(),
      ...
      jest.spyOn(dailyChecksService, 'findByToken').mockResolvedValue(dailyCheck as any);
    ...
    it('should return already responded message if response exists', async () => {
      ...
      jest.spyOn(dailyChecksService, 'findByToken').mockResolvedValue(respondedCheck as any);
    ...
    it('should handle direct response via query parameter', async () => {
      ...
      jest.spyOn(dailyChecksService, 'findByToken').mockResolvedValue(dailyCheck as any);
      jest.spyOn(dailyChecksService, 'updateResponse').mockResolvedValue({ response: 'clean' } as any);
  ...
  describe('/trustman/response/:token (POST)', () => {
    it('should record clean response', async () => {
      ...
      const updatedCheck = { ...dailyCheck, response: 'clean', respondedAt: new Date() };

      jest.spyOn(dailyChecksService, 'findByToken').mockResolvedValue(dailyCheck as any);
      jest.spyOn(dailyChecksService, 'updateResponse').mockResolvedValue(updatedCheck as any);
    ...
    it('should record drank response and mark bet as failed', async () => {
      ...
      const updatedCheck = { ...dailyCheck, response: 'drank', respondedAt: new Date() };
